...
public class Searchtrain implements ActionListener{
  ...
  static JFrame frame; 
  JButton BuyTicket;
  static JButton []trainButton= new JButton[4];

 public Searchtrain(Person p){
  person=p;
...
public static void main(String[] args) {
    Person person = new Person("01773259873","1","shuvo", "783");
...
public void actionPerformed(ActionEvent e) {
    ...
    if(e.getSource()==trainButton[0]){
        if(trainButton[0].getText()!="No More Train"){
            DirectingToSeatpage(person, trainFromButton[0], date, sType, from, to);
        }
    }
    if(e.getSource()==trainButton[1]){
        if(trainButton[1].getText()!="No More Train"){
            DirectingToSeatpage(person, trainFromButton[1], date, sType, from, to);
        }
    }
    if(e.getSource()==trainButton[2]){
        if(trainButton[2].getText()!="No More Train"){
            DirectingToSeatpage(person, trainFromButton[2], date, sType, from, to);
        }
    }
    if(e.getSource()==trainButton[3]){
        if(trainButton[3].getText()!="No More Train"){
            DirectingToSeatpage(person, trainFromButton[3], date, sType, from, to);
        }
    }
...
public static void DirectingToSeatpage( Person p, Train tr, String date, String gc, String f, String t ){
    if(gc=="SHOVAN"){
        frame.setVisible(false);
        new ShovanSeat(p, tr, date, gc, f, t );
    }
    if(gc=="S_CHAIR"){
        frame.setVisible(false);
        new S_Chair(p, tr, date, gc, f, t );
    }
    if(gc=="AC_BERTH"){
        frame.setVisible(false);
        new AcSeat(p, tr, date, gc, f, t );
    }
    if(gc=="SNIGDHA"){
        frame.setVisible(false);
        new SnigdhaSeat(p, tr, date, gc, f, t );
    }
}
